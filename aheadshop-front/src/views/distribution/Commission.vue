<template>
  <div class="distribution-page">
    <div class="dist-container">
      <div class="page-header">
        <h1>佣金明细</h1>
      </div>

      <!-- 佣金汇总 -->
      <div class="summary-cards">
        <div class="sum-card">
          <div class="sum-label">累计佣金</div>
          <div class="sum-value">¥{{ summary?.totalCommission?.toFixed(2) || '0.00' }}</div>
        </div>
        <div class="sum-card highlight">
          <div class="sum-label">可提现</div>
          <div class="sum-value">¥{{ summary?.availableCommission?.toFixed(2) || '0.00' }}</div>
        </div>
        <div class="sum-card">
          <div class="sum-label">冻结中</div>
          <div class="sum-value">¥{{ summary?.frozenCommission?.toFixed(2) || '0.00' }}</div>
        </div>
      </div>

      <!-- 状态筛选 -->
      <div class="table-card">
        <el-tabs v-model="activeStatus" @tab-change="handleTabChange">
          <el-tab-pane label="全部" name="all" />
          <el-tab-pane label="冻结中" name="0" />
          <el-tab-pane label="已结算" name="1" />
          <el-tab-pane label="已取消" name="2" />
        </el-tabs>

        <el-table :data="commissionList" v-loading="loading" empty-text="暂无佣金记录" stripe>
          <el-table-column label="订单号" prop="orderNo" min-width="180" />
          <el-table-column label="订单金额" width="120">
            <template #default="{ row }">
              ¥{{ row.orderAmount?.toFixed(2) }}
            </template>
          </el-table-column>
          <el-table-column label="佣金比例" width="100">
            <template #default="{ row }">
              {{ (row.commissionRate * 100).toFixed(0) }}%
            </template>
          </el-table-column>
          <el-table-column label="佣金金额" width="120">
            <template #default="{ row }">
              <span class="amount-highlight">+¥{{ row.commissionAmount?.toFixed(2) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="状态" width="100">
            <template #default="{ row }">
              <el-tag :type="CommissionStatusMap[row.status]?.type as any" size="small">
                {{ CommissionStatusMap[row.status]?.label }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column label="结算时间" width="180">
            <template #default="{ row }">
              {{ row.settleTime?.replace('T', ' ')?.substring(0, 19) || '-' }}
            </template>
          </el-table-column>
          <el-table-column label="创建时间" prop="createTime" width="180">
            <template #default="{ row }">
              {{ row.createTime?.replace('T', ' ')?.substring(0, 19) || '-' }}
            </template>
          </el-table-column>
        </el-table>

        <div class="pagination" v-if="total > pageSize">
          <el-pagination
            v-model:current-page="pageNum"
            :page-size="pageSize"
            :total="total"
            layout="total, prev, pager, next"
            @current-change="loadData"
          />
        </div>
      </div>

      <div class="back-link">
        <router-link to="/distribution">← 返回分销中心</router-link>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { getCommissionSummary, getCommissionList } from '@/api/distribution'
import type { CommissionSummary, CommissionRecord } from '@/types/distribution'
import { CommissionStatusMap } from '@/types/distribution'

const summary = ref<CommissionSummary | null>(null)
const commissionList = ref<CommissionRecord[]>([])
const loading = ref(false)
const activeStatus = ref('all')
const pageNum = ref(1)
const pageSize = 10
const total = ref(0)

const loadData = async () => {
  loading.value = true
  try {
    const params: Record<string, any> = { pageNum: pageNum.value, pageSize }
    if (activeStatus.value !== 'all') {
      params.status = Number(activeStatus.value)
    }
    const res = await getCommissionList(params as any)
    commissionList.value = res?.records || []
    total.value = res?.total || 0
  } catch {
    // ignore
  } finally {
    loading.value = false
  }
}

const handleTabChange = () => {
  pageNum.value = 1
  loadData()
}

onMounted(async () => {
  try {
    const res = await getCommissionSummary()
    summary.value = res
  } catch {
    // ignore
  }
  loadData()
})
</script>

<style scoped>
.distribution-page {
  min-height: 100vh;
  background: var(--color-cream, #fdfbf7);
  padding: 24px 0 80px;
}

.dist-container {
  max-width: 900px;
  margin: 0 auto;
  padding: 0 20px;
}

.page-header {
  margin-bottom: 24px;
}

.page-header h1 {
  font-family: var(--font-display, 'Playfair Display', serif);
  font-size: 28px;
  color: var(--color-charcoal, #2d2926);
  margin: 0;
}

/* 汇总卡片 */
.summary-cards {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 12px;
  margin-bottom: 20px;
}

.sum-card {
  background: #fff;
  border-radius: var(--radius-lg, 12px);
  padding: 16px;
  text-align: center;
  box-shadow: var(--shadow-sm, 0 1px 3px rgba(0,0,0,0.06));
}

.sum-card.highlight {
  background: linear-gradient(135deg, var(--color-charcoal, #2d2926), #4a4543);
  color: #fff;
}

.sum-label {
  font-size: 12px;
  color: #999;
  margin-bottom: 6px;
}

.sum-card.highlight .sum-label {
  color: rgba(255,255,255,0.7);
}

.sum-value {
  font-size: 20px;
  font-weight: 700;
  font-family: var(--font-display, 'Playfair Display', serif);
  color: var(--color-charcoal, #2d2926);
}

.sum-card.highlight .sum-value {
  color: #fff;
}

/* 表格 */
.table-card {
  background: #fff;
  border-radius: var(--radius-lg, 12px);
  padding: 20px;
  box-shadow: var(--shadow-sm, 0 1px 3px rgba(0,0,0,0.06));
}

.amount-highlight {
  font-weight: 700;
  color: var(--color-amber, #d4a574);
}

.pagination {
  display: flex;
  justify-content: center;
  margin-top: 20px;
}

.back-link {
  margin-top: 20px;
  text-align: center;
}

.back-link a {
  color: var(--color-amber, #d4a574);
  text-decoration: none;
  font-size: 14px;
}

.back-link a:hover {
  text-decoration: underline;
}

@media (max-width: 600px) {
  .summary-cards {
    grid-template-columns: 1fr;
  }
}
</style>
